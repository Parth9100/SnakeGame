...
extern Color gameboyGreen, mediumGreen;
...
void StartMenu::Draw() {
	...
	DrawText(subtitle, (screenWidth - subTitleWidth) / 2, screenHeight / 4 + 80, subtitleFontSize, mediumGreen);
	...
	for (int i = 0; i < MENU_OPTIONS_COUNT; i++) {
		...
		if (i == selectedOption) {
			DrawRectangle((screenWidth - optionWidth) / 2 - 20, menuStartY + i * menuSpacing - 5, optionWidth + 40, menuFontSize + 10, mediumGreen);
		}
	...
	int controlsFontSize = 16;
	int controlsWidth = MeasureText(controls, controlsFontSize);
	DrawText(controls, (screenWidth - controlsWidth) / 2, screenHeight - 100, controlsFontSize, mediumGreen);
